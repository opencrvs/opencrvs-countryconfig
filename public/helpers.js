...
window.getRegionName = window.getChildNID = window.isFirstCertificate = window.mentions = window.getPlaceOfBirth = window.customizeDistrictNameLocation = window.customizeOfficeNameLocationHeader = window.replaceByUppercaseLocation = window.definitionDistrictInTheAct = window.definitionCommuneInTheAct = window.customizeOfficeNameLocation = window.birthCertificateRelatedPerson = window.translateDateToMDGFormat = window.getBirthRegistrationDateMDGFormat = window.translateTime = window.customizeChildBirthRegistrationNumber = window.translateDateYear = window.translateDate = window.signatureDescription = window.registrationStatement = window.eventStatement = window.introduction = window.numberOfTimesCertificatePrinted = window.linebreak = window.foreignObject = window.text = window.join = window.wrapGroup = window.wrap = void 0;
var LINE_HEIGHT = 15;
...
function wordWrap(text, boundary, fontSize, fontWeight) {
    var letterSizeFactor = (fontSize != null ? (fontSize + 1.13) / DEFAULT_FONTSIZE : 1) +
...
function insertTspansIntoText(textLines, xi, yi) {
    ...
    for (var _i = 0, textLines_1 = textLines; _i < textLines_1.length; _i++) {
        ...
        svgString += "<tspan x=\"".concat(xi, "\" y=\"").concat(y, "\">").concat(line, "</tspan>");
        y += LINE_HEIGHT;
    }
    return svgString;
}
    ...
    return function (options) {
        function insertTspansIntoText(textLines) {
            ...
            for (var _i = 0, textLines_2 = textLines; _i < textLines_2.length; _i++) {
                ...
                if (lineTrimed !== '') {
                    svgString += "<tspan x=\"".concat(this.x, "\" y=\"").concat(this.y, "\">").concat(lineTrimed == DOUBLE_LINE_BREAk_SEPARATION.trim() ? '' : lineTrimed, "</tspan>");
                    this.y += LINE_HEIGHT;
                }
            }
            return svgString;
...
function foreignObject() {
    return function (width, height, options) {
        var content = options.fn(this);
        var res = "<foreignObject x=\"".concat(this.x, "\" y=\"").concat(this.y - 10, "\" width=\"").concat(width, "\" height=\"").concat(height, "\">").concat(content, "</foreignObject>");
        this.y += height;
        return res;
    };
}
...
function eventStatement() {
    return function (placeOfBirthState, placeOfBirthDistrict, fatherPrimaryDistrict, motherPrimaryDistrict) {
        return joinValuesWith(__spreadArray(__spreadArray([
            "--Tamin'ny",
...
function fatherDetails(fatherPrimaryDistrict) {
    ...
    return joinValuesWith([
        joinValuesWith(__spreadArray(__spreadArray([
            joinValuesWith([this.fatherFamilyName, this.fatherFirstName]) + ',',
            // 'rainy,',
            'teraka tamin’ny',
            this.birthFatherCustomizedExactDateOfBirthUnknown
                ? convertNumberToLetterForMalagasySpecificLanguage(parseInt(this.birthFatherYearOfBirth))
...
function motherDetails(motherPrimaryDistrict) {
    ...
    return joinValuesWith(__spreadArray(__spreadArray([
        ...
        this.birthMotherCustomizedExactDateOfBirthUnknown
            ? convertNumberToLetterForMalagasySpecificLanguage(parseInt(this.birthMotherYearOfBirth))
...
function convertToTimeZoneIso(dateUtc, timeZone) {
    ...
    return "".concat(year, "-").concat(month, "-").concat(day, "T").concat(hour == '24' ? '00' : hour, ":").concat(minute, ":").concat(second, ".").concat(milliseconds, "+03:00");
}
function registrationStatement() {
    return function (informantPrimaryDistrict, registrationDistrict) {
        ...
        var registrarDateUTC = convertToTimeZoneIso(isValidDate(birthRegistrationDate)
            ? birthRegistrationDate
            : this.registrar.date, 'Africa/Nairobi');
        return joinValuesWith(__spreadArray(__spreadArray([
            '---Nosoratana androany',
            ...
            (relationMap[(_a = this.informantType) === null || _a === void 0 ? void 0 : _a.toLowerCase()] || 'mpanolotra') + ','
        ], (isInformantMotherOrFather(this.informantType)
            ? []
            : __spreadArray(__spreadArray([
                "teraka tamin'ny",
                this.birthInformantCustomizedExactDateOfBirthUnknown
                    ? convertNumberToLetterForMalagasySpecificLanguage(parseInt(this.birthInformantYearOfBirth))
                ...
                ? [
                    ...
                    this.birthInformantFokontanyCustomAddress
                        ? "fokontany ".concat(this.birthInformantFokontanyCustomAddress, ",")
                        : '',
                    'kaominina',
                    (definitionOffice(replaceByUppercase(informantPrimaryDistrict)) || '-') + ','
                ]
                : [
                    ([
...
function signatureDescription() {
    return function () {
        return joinValuesWith([
            ...
            customizeDateInCertificateContent(new Date().toISOString().split('T')[0]) + ", ary nomena an'i ",
            joinValuesWith([this.informantFamilyName, this.informantFirstName], ' ') + '--'
        ], ' ');
    };
}
...
var convertNumberToLetterForMalagasySpecificLanguage = function (num) {
    ...
    if (digitLength.length > 9)
        return 'mihoatra lavitra';
        ...
        digits[5] != '00'
            ? (unit[Number(digits[5])] ||
                (unit[parseInt(digits[5][1])] != ''
                    ? "".concat(unit[parseInt(digits[5][1])], " amby  ").concat(DOZENS_MDG_WORDS[parseInt(digits[5][0])])
...
function convertTimeToMdgCustomWords(timeString) {
    var _a = timeString.split(':'), hour = _a[0], minute = _a[1];
    ...
    if (newHour <= 12 && newHour >= 10)
        timePeriod = 'antoandro';
    else if (newHour > 12) {
        if (newHour <= 16 && newHour >= 13)
            timePeriod = 'tolakandro';
        if (newHour <= 19 && newHour >= 17)
            timePeriod = 'hariva';
        if (newHour < 24 && newHour >= 20)
            timePeriod = 'alina';
        newHour = newHour - 12;
    }
    else if (newHour === 0 && newMinute > 0) {
        return "roa ambin'ny folo ora sy ".concat(convertNumberToLetterForMalagasySpecificLanguage(newMinute), " minitra alina");
    }
    else if (newHour === 0 && newMinute === 0) {
        return "roa ambin'ny folo ora alina";
    }
    var res = "".concat(mdgHours[newHour]).concat(newMinute > 0
        ? " ora sy ".concat(convertNumberToLetterForMalagasySpecificLanguage(newMinute), " ")
        : 'ora').concat(newMinute > 0 ? "minitra " : '', " ").concat(timePeriod);
    return res;
}
...
function convertDateToMdgCustomWords(dateString) {
    ...
    var dateValue = parseInt(day) === 1
        ? "voalohan'ny volana"
        : "".concat(convertNumberToLetterForMalagasySpecificLanguage(parseInt(day)));
    return "".concat(dateValue, " ").concat(THE_MONTH_MDG_WORDS[parseInt(month)], ", taona ").concat(convertNumberToLetterForMalagasySpecificLanguage(parseInt(year)));
}
function convertLocaleDateToMdgCustomWords(dateString) {
    var dateStringStr = dateString.split(' ');
    var _a = dateStringStr[0].split('/'), day = _a[0], month = _a[1], year = _a[2];
    return "".concat(day, " ").concat(THE_MONTH_MDG_WORDS[parseInt(month)], " ").concat(year);
}
...
function customizeChildBirthRegistrationNumber() {
    return function (rawChildBirthRegistrationNumber, manualChildBirthRegistrationNumber) {
        if (manualChildBirthRegistrationNumber === void 0) { manualChildBirthRegistrationNumber = ''; }
        if (manualChildBirthRegistrationNumber &&
            !['', undefined, 'undefined', null, 'null'].includes(manualChildBirthRegistrationNumber.trim()))
            return manualChildBirthRegistrationNumber.toString().padStart(6, '0');
        var currentRegisterNumber = '';
        if (rawChildBirthRegistrationNumber &&
...
window.customizeChildBirthRegistrationNumber = customizeChildBirthRegistrationNumber;
function removeOrdinalIndicator(dateString) {
    var _a;
    if (!dateString)
        return new Date(); // TODO: Handle it later
    return dateString && ((_a = dateString === null || dateString === void 0 ? void 0 : dateString.replace) === null || _a === void 0 ? void 0 : _a.call(dateString, /\b(\d+)(th|st|nd|rd)\b/g, '$1'));
}
...
function getBirthRegistrationDateMDGFormat() {
    return function () {
        ...
        if (isValidDate(birthRegistrationDate)) {
            return handleTranslateDateToMDGFormat(this.birthChildLegacyBirthRegistrationDate);
        }
...
function translateChildGenderToMDGWord(childGender) {
    return childGender &&
        ['male', 'homme', 'lehilahy', 'zazalahy'].includes(childGender.toLowerCase())
        ? 'zazalahy'
        : 'zazavavy';
}
function birthCertificateRelatedPerson() {
    return function (informantFamilyName) {
        if (informantFamilyName === void 0) { informantFamilyName = ''; }
        var informantInfo = '';
        if (informantFamilyName === null || informantFamilyName === void 0 ? void 0 : informantFamilyName.trim()) {
            informantInfo +=
                this.motherFamilyName === informantFamilyName &&
                    ...
                    : this.fatherFamilyName === informantFamilyName &&
                        ...
                        ? 'ny rainy'
                        : "i ".concat(informantFamilyName, " ").concat(this.informantFirstName);
        }
...
function definitionOffice(officeName) {
    if (officeName === void 0) { officeName = ''; }
    var locationMappings = {
...
function replaceAbbreviations(inputText) {
    ...
    var regex = new RegExp("\\b(".concat(Object.keys(abbreviationMap).join('|'), ")\\b"), 'g');
    // Remplacer les abréviations par leurs définitions
    return inputText.replace(regex, function (match) { return abbreviationMap[match]; });
}
...
function customizeDistrictNameLocation() {
    return function (placeOfBirthDistrict) {
        if (placeOfBirthDistrict === void 0) { placeOfBirthDistrict = ''; }
...
function getPlaceOfBirth(placeOfBirthFacility, fokontanyCustomAddress) {
    if (placeOfBirthFacility === void 0) { placeOfBirthFacility = ''; }
    if (fokontanyCustomAddress === void 0) { fokontanyCustomAddress = ''; }
    return !['', ' ', null, 'null', 'undefined', undefined].includes(fokontanyCustomAddress)
...
function getRecognitionMentionValues(i) {
    var _a, _b, _c, _d, _e, _f;
    if (!((_c = (_b = (_a = this['birthMentionTypeOfMention__' + i]) === null || _a === void 0 ? void 0 : _a.toString()) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === null || _c === void 0 ? void 0 : _c.includes('recognition'))) {
        return [];
    }
    return [
        "Nozanahan'i ".concat(this['birthMentionChildFamilyName__' + i] || '-', " ").concat(this['birthMentionChildFirstName__' + i] || '-', " tamin'ny ").concat(((_f = (_e = (_d = this['birthMentionRecognitionDate__' + i]) === null || _d === void 0 ? void 0 : _d.split('-')) === null || _e === void 0 ? void 0 : _e.reverse()) === null || _f === void 0 ? void 0 : _f.join('/')) || '-', " tao amin'ny kaominina ").concat(this['birthMentionRecognitionPlace__' + i] || '-', " soratra faha ").concat(this['birthMentionRecognitionActNumber__' + i] || '-', ".")
    ];
...
function mentions() {
    return function () {
        ...
        for (var i = 0; i < 10; i++) {
            ...
            var temp = __spreadArray(__spreadArray([], getRecognitionMentionValues.apply(this, [i]), true), [
...
function getRegionName() {
    return function (districtName) {
        var _a;
        return (_a = districts.find(function (d) { return d.name === districtName.toUpperCase(); })) === null || _a === void 0 ? void 0 : _a.region_name;
    };
}
