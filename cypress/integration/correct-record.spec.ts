...
context('Search Integration Test', () => {
  ...
  it("declaration can be found with child's name", () => {
    ...
    cy.get('#assign').click()
    cy.get('#name_0').click()
    cy.get('[data-testid=trackingId-value]').invoke('text').then((trackingId) => {
        cy.get('#btn-correct-record').click()
        cy.get('#relationship-form-input').contains('Another registration agent or field agent').click()
