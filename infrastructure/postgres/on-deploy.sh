...
: "${EVENTS_MIGRATOR_ANALYTICS_POSTGRES_PASSWORD:?Must set EVENTS_MIGRATOR_ANALYTICS_POSTGRES_PASSWORD}"
: "${EVENTS_APP_ANALYTICS_POSTGRES_PASSWORD:?Must set EVENTS_APP_ANALYTICS_POSTGRES_PASSWORD}"
...
DB_EXISTS=$(PGPASSWORD="$ANALYTICS_POSTGRES_PASSWORD" psql -qtAX -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" \
  -U "$ANALYTICS_POSTGRES_USER" -d postgres \
...
if [[ "$DB_EXISTS" == "1" ]]; then
  ...
  PGPASSWORD="$ANALYTICS_POSTGRES_PASSWORD" psql -v ON_ERROR_STOP=1 -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" \
    -U "$ANALYTICS_POSTGRES_USER" -d postgres <<EOF
ALTER ROLE events_migrator WITH PASSWORD '${EVENTS_MIGRATOR_ANALYTICS_POSTGRES_PASSWORD}';
ALTER ROLE events_app WITH PASSWORD '${EVENTS_APP_ANALYTICS_POSTGRES_PASSWORD}';
EOF
...
PGPASSWORD="$ANALYTICS_POSTGRES_PASSWORD" psql -v ON_ERROR_STOP=1 -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" \
  -U "$ANALYTICS_POSTGRES_USER" -d postgres <<EOF || { echo "❌ Cluster-wide SQL failed"; exit 1; }
CREATE DATABASE "$TARGET_DB";

CREATE ROLE events_migrator WITH LOGIN PASSWORD '${EVENTS_MIGRATOR_ANALYTICS_POSTGRES_PASSWORD}';
CREATE ROLE events_app WITH LOGIN PASSWORD '${EVENTS_APP_ANALYTICS_POSTGRES_PASSWORD}';
...
PGPASSWORD="$ANALYTICS_POSTGRES_PASSWORD" psql -v ON_ERROR_STOP=1 -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" \
  -U "$ANALYTICS_POSTGRES_USER" -d "$TARGET_DB" <<EOF || { echo "❌ DB-specific SQL failed"; exit 1; }
