 ...
 *     that audience);
 *   - the auth service pre-authentication flows (2fa, reset-password, username
 *     reminder) mint a token with the `opencrvs:countryconfig-user` audience.
 * Leaving them unauthenticated would let anyone trigger 2FA codes, password
 * reset credentials and notification emails/SMS to arbitrary recipients.
 *
