...
describe('Token verifier module', () => {
  describe('.verifyToken()', () => {
    it('Calls the search service and return true if valid', async () => {
      ...
      const valid = await verifyToken('111', 'http://search.opencrvs.org')
      expect(valid).toBe(true)
    })

    it('Calls the search service and return false if valid', async () => {
      fetch.mockResponseOnce(
        JSON.stringify({
          valid: false
        })
      ...
      const valid = await verifyToken('222', 'http://search.opencrvs.org')
