---
applyTo: "src/events/**"
description: "Use when creating or modifying event definitions, declaration forms, or form pages in src/events/"
---
...
Use `src/events/birth/` as the canonical reference.  
Use `src/events/tennis-club-membership.ts` as the canonical **custom** event reference.
...
| Type | Behaviour |
|------|-----------|
| `ConditionalType.SHOW` | Show field when `conditional` is true; hide when false |
...
field('section.name').isNotEqualTo(value)
...
| Context | Pattern |
|---------|---------|
| Form field label | `event.{eventId}.action.declare.form.section.{page}.field.{fieldId}.label` |
| Validation error | `error.{errorName}` |
| User role label | `userRole.{camelCaseRoleId}` |
| Select option label | `form.field.label.{optionContext}{OptionValue}` |
| Event top-level | `event.{eventId}.label` / `event.{eventId}.summary.title` |

**Rule**: Every new label `id` must have a corresponding row in [src/translations/client.csv](../../src/translations/client.csv).  
CSV columns: `id,description,en`
