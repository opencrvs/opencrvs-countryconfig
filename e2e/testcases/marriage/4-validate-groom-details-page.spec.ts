...
test.describe("4. Validate the groom's details page", () => {
  ...
  test.describe('2 Validate the National ID field', async () => {
    test('2.1. Enter numerical value less than 9 digit', async ({ page }) => {
      ...
      await page.locator('#groomNationalId').fill(shortValue)
    ...
    test('2.2 Set the field as NULL', async ({ page }) => {
      await page.locator('#groomIdType').click()
      await page.getByText('National ID', { exact: true }).click()
      await page.locator('#groomNationalId').click()
      await page.getByText("Groom's details").click()
      await expect(page.locator('#groomNationalId_error')).toHaveText(
    ...
    test('2.3 Enter numerical value 9 digit', async ({ page }) => {
      ...
      await page.locator('#groomIdType').click()
      await page.getByText('National ID', { exact: true }).click()
      await page.locator('#groomNationalId').fill(correctValue)
      await page.getByText("Groom's details").click()

      await expect(page.locator('#groomNationalId_error')).toBeHidden()
    })
    ...
    test('2.4 Enter numerical value above 9 digit', async ({ page }) => {
      ...
      await page.locator('#groomIdType').click()
      await page.getByText('National ID', { exact: true }).click()
      await page.locator('#groomNationalId').fill(tooLongValue)
      await page.getByText("Groom's details").click()
    ...
    test('2.5 Enter the same 9 digit value of informant', async ({ page }) => {
      ...
      await page.locator('#groomIdType').click()
      await page.getByText('National ID', { exact: true }).click()
      await page.locator('#groomNationalId').fill(sameNumbersValue)
      await page.getByText("Groom's details").click()
  ...
  test.describe('2.1 Validate "First Name(s)" text field', async () => {
    ...
    test('2.1.3 Enter Field as NULL', async ({ page }) => {
       ...
       */
      await expect(
        page.locator('#required_label_groom_firstNamesEng')
      ).toBeVisible()
    })
  ...
  test.describe('3 Validate the "DOB" field', async () => {
    test('3.1. Enter date Less than the current date and focus out from the field ', async ({
       ...
       */
      await expect(page.locator('#groomBirthDate_error')).toBeHidden()
    })
    ...
    test('3.2. Enter invalid date', async ({ page }) => {
       ...
       */
      await expect(page.locator('#groomBirthDate_error')).toHaveText(
    ...
    test('2.3 Set the field as null', async ({ page }) => {
       ...
       */
      await expect(
        page.locator('#required_label_groom_groomBirthDate')
