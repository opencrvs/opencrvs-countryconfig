...
test.describe.serial(
  'Correct record - change informant type',
  { tag: '@cihan' },
  () => {
    ...
    const visible = async (
      ...
      col2 &&
        (await expect(_page.getByText(col2, { exact: true })).toBeVisible())
      col3 &&
        (await expect(_page.getByText(col3, { exact: true })).toBeVisible())
    }
    ...
    test('4.9 Validate record corrected modal', async () => {
      ...
      const date = await correctionRequestedRow
        .locator('span')
        .nth(1)
        .innerText()
