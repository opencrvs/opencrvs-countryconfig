...
- **Clear button on form pages**: set `showClearButton: true` on a page to show a "Clear" button beside its title, which resets the page's fields after a confirmation. Requires `@opencrvs/toolkit` `1.9.16`. [#10135](https://github.com/opencrvs/opencrvs-core/issues/10135)
- **Direct authentication for integrations**: integrations listed in `INTEGRATIONS` in `src/api/integration/handler.ts` are registered with user management on startup through the new `GET /triggers/system/ready` endpoint, and authenticate with their own credentials rather than the requesting user's token. Credentials are generated by OpenCRVS, never stored in the country configuration, and revealed to a National System Admin under Configurations → Integrations. The list ships empty, so an unmodified configuration is unaffected. Because these actions have no role, `analytics.event_actions.created_by_role` is now nullable; the constraint is dropped on existing databases during deploy. [#12360](https://github.com/opencrvs/opencrvs-core/issues/12360)

### Breaking changes

- Add `USER_MANAGEMENT_URL=http://user-mgnt:3030` to the `countryconfig` service in `infrastructure/docker-compose.deploy.yml`. It has no production default, so the service will not start without it. [#12360](https://github.com/opencrvs/opencrvs-core/issues/12360)
