...
- **Removed metabase backup & restore** volume(/data/metabase) mount in deployment configuration(docker compose files) from `dashboard` service. We are no longer creating backup for metabase and also we are no longer restoring metabase. In ansible playbooks, removed all `data/metabase/` related setup. [#8043](https://github.com/opencrvs/opencrvs-core/issues/8043)
